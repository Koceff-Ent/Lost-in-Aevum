...
namespace TheVegetationEngine
{
    ...
    public class TVEGlobalMotion : StyledMonoBehaviour
    {
        ...
        {
            Shader.SetGlobalVector("TVE_MotionDirection", gameObject.transform.forward);
            Shader.SetGlobalFloat("TVE_MotionAmplitude_10", Mathf.Lerp(0.1f, 1f, windPower) * (int)windMode);
            Shader.SetGlobalFloat("TVE_MotionAmplitude_20", Mathf.Lerp(0.25f, 1f, windPower) * (int)windMode);
            Shader.SetGlobalFloat("TVE_MotionAmplitude_21", Mathf.Lerp(0.1f, 1f, windPower) * (int)windMode);
            Shader.SetGlobalFloat("TVE_MotionAmplitude_30", Mathf.Lerp(0.25f, 1f, windPower) * (int)windMode);
            Shader.SetGlobalFloat("TVE_MotionAmplitude_32", Mathf.Lerp(0.25f, 1f, windPower) * (int)windMode);

            Shader.SetGlobalTexture("TVE_NoiseTex", noiseTexture);
            Shader.SetGlobalFloat("TVE_NoiseContrast", Mathf.Lerp(0.25f, 2.5f, 1 - windPower) * (int)windMode);
